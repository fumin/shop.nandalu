module ZMQ
  ...
  module LibZMQ
    ...
    begin
      ...
      a_path = [inside_gem, *lib_paths].map{|path| "#{path}/libzmq.#{FFI::Platform::LIBSUFFIX}"}
      b_path = [inside_gem, *lib_paths].map{|path| "#{path}/libzmq.a"}
      ZMQ_LIB_PATHS = a_path.concat(b_path)
